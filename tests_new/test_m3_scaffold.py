...
# --- M3-AC3: non-M3 subcommands raise NotImplementedError with milestone pointer ---
...
def test_non_m3_subcommands_raise_not_implemented(subcommand: str, milestone: str) -> None:
    from engrish.cli import main as cli_main

    with pytest.raises(NotImplementedError) as exc:
        cli_main([subcommand])
    message = str(exc.value)
    assert subcommand in message, f"error must name the subcommand ({subcommand})"
    assert milestone in message, f"error must name the owning milestone ({milestone})"
